...
class PMG(nn.Module):
    ...
    def forward(self, x, is_train=True):
        ...
        top_n_index = top_n_cdds[:, :, -1].astype(np.int32)
        ...
        for i in range(batch):
            for j in range(self.topn):
                [y0, x0, y1, x1] = top_n_cdds[i, j, 1:5].astype(np.int32)
        ...
        yp4 = self.classifier_concat(
            torch.cat((f1_part, f2_part, f3_part), -1))

        # resort parts
        feature_points = f3_part.view(batch, self.topn, -1)
        parts_order = self.PR.classify(
            feature_points.data.cpu().numpy(), is_train)
        parts_order = torch.from_numpy(parts_order).long().to(x.device)
        parts_order = parts_order.unsqueeze(2).expand(
            batch, self.topn, self.num_ftrs//2)

        f1_points = torch.gather(f1_part.view(
            batch, self.topn, -1), dim=1, index=parts_order)
        f1_m = self.reg_mlp1(f1_points.view(batch, -1))
        f2_points = torch.gather(f2_part.view(
            batch, self.topn, -1), dim=1, index=parts_order)
        f2_m = self.reg_mlp2(f2_points.view(batch, -1))
        f3_points = torch.gather(f3_part.view(
            batch, self.topn, -1), dim=1, index=parts_order)
