...
def load_model(backbone, pretrain=True, require_grad=True, im_sz=448, classes_num=200, topn=4):
    ...
    if backbone == 'resnet50':
        ...
        net = PMG(net, im_sz, feature_size, num_ftrs, classes_num, topn=topn)
    elif backbone == 'resnet101':
        num_ftrs = 2048
        net = resnet101(pretrained=pretrain)
        for param in net.parameters():
            param.requires_grad = require_grad
        net = PMG(net, im_sz, feature_size, num_ftrs, classes_num)
...
def test(net, testset, batch_size):
    ...
    testloader = torch.utils.data.DataLoader(
        testset, batch_size=batch_size//2, shuffle=False, num_workers=num_workers, drop_last=False)
    ...
    for _, (inputs, targets) in enumerate(testloader):
        ...
        y1, y2, y3, y4, _, _, _, _, _, _, _, _, _, _, _ = net(
...
def smooth_CE(logits, label, peak):
    ...
    label_logits = np.zeros(
        logits.shape, dtype=np.float32) + (1-peak)/(num_cls-1)
